...
public interface ChannelRepository extends JpaRepository<Channel, Long> {
    @Query(
        value = "select distinct channel from Channel channel left join fetch channel.posts",
    ...
    @Query("select distinct channel from Channel channel left join fetch channel.posts")
    List<Channel> findAllWithEagerRelationships();

    @Query("select channel from Channel channel left join fetch channel.posts where channel.id =:id")
