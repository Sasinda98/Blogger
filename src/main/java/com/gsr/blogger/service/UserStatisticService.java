...
public class UserStatisticService {
    ...
    /**
     *  Get all the userStatistics where MUser is {@code null}.
     *  @return the list of entities.
     */
    @Transactional(readOnly = true)
    public List<UserStatisticDTO> findAllWhereMUserIsNull() {
        log.debug("Request to get all userStatistics where MUser is null");
        return StreamSupport
            .stream(userStatisticRepository.findAll().spliterator(), false)
            .filter(userStatistic -> userStatistic.getMUser() == null)
            .map(userStatisticMapper::toDto)
