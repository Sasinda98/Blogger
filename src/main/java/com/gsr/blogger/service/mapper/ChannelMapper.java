...
public interface ChannelMapper extends EntityMapper<ChannelDTO, Channel> {
    @Mapping(target = "relatedPosts", source = "relatedPosts", qualifiedByName = "idSet")
    @Mapping(target = "channelCreator", source = "channelCreator", qualifiedByName = "id")
    ChannelDTO toDto(Channel s);

    @Mapping(target = "removeRelatedPost", ignore = true)
