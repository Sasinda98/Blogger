...
public class UserStatisticResource {
    ...
    public List<UserStatisticDTO> getAllUserStatistics(@RequestParam(required = false) String filter) {
        if ("linkeduser-is-null".equals(filter)) {
            log.debug("REST request to get all UserStatistics where linkedUser is null");
            return userStatisticService.findAllWhereLinkedUserIsNull();
        }
