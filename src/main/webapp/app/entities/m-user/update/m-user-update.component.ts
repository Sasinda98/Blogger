...
export class MUserUpdateComponent implements OnInit {
  ...
  userStatisticsCollection: IUserStatistic[] = [];
  ...
  protected updateForm(mUser: IMUser): void {
    this.editForm.patchValue({
      ...
      about: mUser.about,
      userStatistic: mUser.userStatistic,
    });

    this.userStatisticsCollection = this.userStatisticService.addUserStatisticToCollectionIfMissing(
      this.userStatisticsCollection,
      mUser.userStatistic
    );
  }
  ...
  protected loadRelationshipsOptions(): void {
    this.userStatisticService
      .query({ filter: 'muser-is-null' })
      .pipe(map((res: HttpResponse<IUserStatistic[]>) => res.body ?? []))
      .pipe(
        map((userStatistics: IUserStatistic[]) =>
          this.userStatisticService.addUserStatisticToCollectionIfMissing(userStatistics, this.editForm.get('userStatistic')!.value)
        )
      )
      .subscribe((userStatistics: IUserStatistic[]) => (this.userStatisticsCollection = userStatistics));
  }
  ...
  protected createFromForm(): IMUser {
    return {
      ...
      userStatistic: this.editForm.get(['userStatistic'])!.value,
    };
  }
}
